...
<head>
	...
	<script>
		...
		function loadTools() {
			...
			$("#modal-slider-brightness").slider({
				values: [0],
				min: -255,
				max: 255
			});
			$("#modal-slider-contrast").slider({
				values: [0],
				min: -100,
				max: 100
			});
			$("#modal-slider-smooth").slider({
				values: [0],
				min: -8,
				max: 8
			});
			$("#modal-slider-pixelate").slider({
				values: [0],
				min: 0,
				max: 10
			});
			$("#modal-slider-red").slider({
				values: [0],
				min: -255,
				max: 255
			});
			$("#modal-slider-green").slider({
				values: [0],
				min: -255,
				max: 255
			});
			$("#modal-slider-blue").slider({
				values: [0],
				min: -255,
				max: 255
			});
	...
	<div class="container">
		...
		<ul class="thumbnails">
			...
			<?php foreach ($iterator as $file) : ?>
				...
				<li>
					<div class="thumbnail">
						<img src="<?php echo (new Image())->load($file->getPathname())->dataSrc(); ?>" alt="<?php echo $file->getPathname(); ?>">
					</div>
					<!--
			...
			<span class="span6">
				...
				<div class="modal-tabs">
					...
					<div id="modal-tab-customs" class="modal-tab hide">
						...
						<?php
						$convolutions = get_class_methods(ConvolutionFunctions::class);
						?>
